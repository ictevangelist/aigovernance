...
- **The DPIA screening tool** (`downloads/AI-DPIA-Screening-Tool.html`) — a browser-only screening
  tool, delivered as the reward for a short sign-up (Google Form) on `dpia-tool/`.
...
| `landscape/` | Why govern AI + scope + the guidance stack (policy §1–2) |
| `roles/` | Roles, responsibilities & oversight (§3) |
| `approved-tools/` | Approved tools & how tools are approved (§4, 4a) |
| `acceptable-use/` | What staff may / must not do (§5–6) |
| `data-protection/` | Data protection & information security (§7) |
| `accuracy-oversight/` | Accuracy, bias & human oversight (§8) |
| `safeguarding/` | Safeguarding, online safety & Prevent (§9) |
| `impact-assessment/` | Impact assessment & product safety (§10) |
| `training/` | Training & awareness (§11) |
| `filtering-monitoring/` | Filtering, monitoring & assurance (§12) |
| `breaches/` | Breaches & serious incidents (§13) |
| `pupil-use/` | Pupil use (optional module) |
| `guidance-map/` | Every framework, what it requires, where it lands |
| `dpia-tool/` | DPIA screening tool — sign-up (Google Form) → download |
| `policy-template/` | Download the policy template |
| `privacy/` | Privacy notice (no cookies/tracking; Google Form sign-up covered) |
...
4. On the Google Form, set the **confirmation message** to include the download link
   ...
   paste the form's embed `<iframe>` into the marked placeholder in `dpia-tool/`.
